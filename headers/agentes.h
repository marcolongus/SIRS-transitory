				...
				ofstream                    &anim          )
{
	...
	#pragma omp parallel for schedule(guided) reduction(+:healthy,infected,refract) num_threads(16)
	for (size_t p=0; p<N; p++){
		...
		//Animacion:
		if (animation and time_step % anim_step == 0){
			for(size_t p=0; p < N; p++){
				anim << system_new[p].x           << " ";
				anim << system_new[p].y           << " ";
				anim << time_step * delta_time       << " ";
